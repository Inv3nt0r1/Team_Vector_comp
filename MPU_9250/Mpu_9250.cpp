...
float MPU_9250::getangle(angle a) {
  switch (a)
  {
    case getyaw:
            yaw   = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]);   
            yaw   *= 180.0f / PI; 
            yaw   -= -0.1; // Declination at Nasik, California is 0 degrees 1 minutes, For pune it is 13.8 
            Serial.print("Yaw :");
            Serial.print(yaw, 2);
            return yaw;
           break;
...
// but is much less computationally intensive---it can be performed on a 3.3 V Pro Mini operating at 8 MHz!
        void MPU_9250:: MadgwickQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
        {
            ...
            // Auxiliary variables to avoid MPU_9250:: repeated arithmetic
            float _2q1mx;
            float _2q1my;
            float _2q1mz;
            float _2q2mx;
 ...
 // measured ones. 
            void MPU_9250:: MPU_9250:: MahonyQuaternionUpdate(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz)
        {
            float q1 = q[0], q2 = q[1], q3 = q[2], q4 = q[3];   // short name local variable for readability
            float norm;
            float hx, hy, bx, bz;
...
//-------------------------------------------------------------------------
 ...
 for(int i=0;i<3;i++)
 {
   ...
   gyroBias[i]=0;
   accelBias[i]=0;
    q[i+1] = 0.0f;    // vector to hold quaternion
 eInt[i] = 0.0f;
 }
 //-----------------------------------------------------------------
...
void MPU_9250::compute(int *final_value )
{
...
/*    display.clearDisplay();     
  ...
  */  
  ...
  //  pitch *= 180.0f / PI;
  //  yaw   *= 180.0f / PI; 
  //  yaw   -= -0.1; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
  //  roll  *= 180.0f / PI;
     
    if(SerialDebug) {
     yawfromfunc = getangle(getyaw);
     float rollfromfunc = getangle(getroll);
     float pitchfromfunc = getangle(getpitch);
    Serial.print("Yaw, Pitch, Roll: ");
    Serial.print(yawfromfunc, 2);
    Serial.print(", ");
    Serial.print(pitchfromfunc, 2);
    Serial.print(", ");
    Serial.println(rollfromfunc, 2);
